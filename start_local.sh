...
ensure_env_var() {
  ...
  printf '%s=%s\n' "$key" "$value" >> "$file"
  echo "[start_local] appended ${key} to $(basename "$file")"
}
...
VITE_LEMON_PEPPER_MODE=0
VITE_LPS_TWO_COL_1M=0
VITE_POLL_MS=30000
VITE_USE_FIXTURES=0
EOF_ENV
  ...
  ensure_env_var frontend/.env.local VITE_API_BASE "http://127.0.0.1:${BACKEND_PORT}"
  ensure_env_var frontend/.env.local VITE_API_URL "http://127.0.0.1:${BACKEND_PORT}"
  ensure_env_var frontend/.env.local VITE_WS_URL "ws://127.0.0.1:${BACKEND_PORT}/ws"
  ensure_env_var frontend/.env.local VITE_EVENTS_ENABLED "true"
  ensure_env_var frontend/.env.local VITE_LEMON_PEPPER_MODE "0"
fi
