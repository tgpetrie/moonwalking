...
- Rabbit glow/reveal is driven by event delegation on `.board-core` in `frontend/src/components/DashboardShell.jsx`:
  - The board alone sets `data-row-hover="1"` and updates `--emit-x/--emit-y`.
  - Rows must never toggle hover or mutate those vars; keeping that contract prevents competing selectors from breaking the glow.
