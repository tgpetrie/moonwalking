...
- Rabbit glow/reveal is driven by event delegation on `.board-core` in `frontend/src/Dashboard.jsx`:
  - The board alone owns the hover emitter and toggles its bunny-hover state.
  - Rows must never toggle hover or mutate the shared hover variables; keeping that contract prevents competing selectors from breaking the glow.
