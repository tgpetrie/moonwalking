...
const Watchlist = ({ onWatchlistChange, topWatchlist, quickview }) => {
  ...
  // Expanded symbol catalog (persisted)
  ...
  useEffect(() => {
    ...
    const ensureCatalog = async () => {
      ...
      let merged = null;
      try {
        // Prefer Coinbase public currencies endpoint (CORS friendly)
        try {
          ...
          if (cJson && Array.isArray(cJson.data)) {
            ...
            cJson.data.forEach((c) => {
              ...
              if (code && isCrypto) set.add(code);
            });
            merged = Array.from(set);
          }
        } catch (_) { /* ignore */ }
        // Fallback to Exchange products for USD markets
        if (!merged) {
          ...
          if (Array.isArray(products)) {
            const set = new Set(allSymbols);
            products.forEach(p => {
              if (p && typeof p.id === 'string' && p.id.toUpperCase().endsWith('-USD')) {
      ...
      if (merged) {
        ...
        try { localStorage.setItem(CATALOG_KEY, JSON.stringify({ symbols: merged, updatedAt: now })); } catch(_){}
      }
      setCatalogLoaded(true);
  ...
  }, [search, watchlist, allSymbols, catalogLoaded, catalogUpdatedAt]);
  ...
  return (
    <div className="flex flex-col space-y-4 w-full max-w-4xl mx-auto h-full min-h-[420px] px-1 sm:px-3 md:px-0 align-stretch">
      {/* Stylized search always visible */}
      <div className="flex flex-col w-full">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search & add coin (e.g. BTC, ETH, AMP)"
            value={search}
        ...
        {search && (
          <div className="w-full bg-black/90 border border-purple-900 rounded-lg shadow-lg z-10 mt-2">
            {searchResults.map(symbol => {
              const inWatch = (Array.isArray(watchlist) ? watchlist : []).includes(symbol);
              return (
                <div
                  key={symbol}
