#!/usr/bin/env bash
# Stop ONLY the instance started by start_orchestrator_background.sh, with clear logs.
...
kill_pid_if_running () {
  local pid_file="$1"; local label="$2"
  if [ -f "$pid_file" ]; then
    local pid
    pid=$(cat "$pid_file" 2>/dev/null || true)
    if [ -n "${pid:-}" ] && ps -p "$pid" >/dev/null 2>&1; then
      info "Killing $label PID $pid from $(basename "$pid_file")"
...
if [ -f "$STATE_JSON" ]; then
  ...
  rm -f "$STATE_JSON"
  ok "Removed state.json"
fi

# Final safety net: known patterns and ports often used
