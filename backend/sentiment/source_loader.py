"""Utilities for loading catalogued sentiment sources from JSON files."""
...
def _validate_entry(entry: Dict, tier_hint: str, *, path: Path, seen_names, seen_urls):
    ...
    endpoint = _clean_url(entry.get("endpoint") or entry.get("url") or entry.get("href"))
    if endpoint:
...
def load_source_catalog(base_path: str | Path | None = None) -> List[Dict]:
    """Return a merged list of every tier JSON file in the catalog."""
    ...
    for json_path in sorted(root.glob("*.json")):
        ...
        for entry in sources:
            merged = _validate_entry(
                entry,
                tier_hint,
                path=json_path,
                seen_names=seen_titles,
                seen_urls=seen_urls,
            )
...
def load_sources(base_path: str | Path | None = None, *, force_reload: bool = False) -> SourceCatalog:
    """Read and validate the source catalog, caching by path."""
