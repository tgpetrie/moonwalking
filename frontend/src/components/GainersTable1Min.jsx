...
export default function GainersTable1Min({ onRowClick }) {
  ...
  const rows = useMemo(() => (Array.isArray(data) ? data.slice(0, 20) : []), [data]);
  ...
  const statusState = error ? 'error' : 'loading';
  const statusMessage = error ? 'Unable to load 1-min gainers' : 'Loading 1-min gainers…';

  return (
    <section data-test="one-min-card" className="w-full">
      {!hasRows && (
        <div className="mb-3">
          <StatusNote state={statusState} message={statusMessage} />
        </div>
      )}
      <div className="relative overflow-x-auto">
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {displayRows.map((coin, index) => (
              <AnimatedRow
                key={`${coin.symbol || 'placeholder'}-${index}`}
                coin={coin}
                index={index}
                changeDirection={hasRows ? changedMap?.get?.(coin.symbol) : undefined}
                changeLabel="1-min Δ%"
                onRowClick={hasRows ? onRowClick : undefined}
              />
            ))}
          </tbody>
        </table>
