...
const ENDPOINT = '/api/component/losers-table';
...
export default function LosersTable({ rows: rowsProp, loading = false, error = null, onRowClick }) {
  ...
  if (loading) {
    return <StatusNote state="loading" message="Loading 3-min losers…" />;
  }

  if ((error || fetchError) && rows.length === 0) {
    return <StatusNote state="error" message="Unable to load 3-min losers" />;
  }
