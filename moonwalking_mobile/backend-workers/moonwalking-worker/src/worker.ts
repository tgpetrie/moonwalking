...
const PRODUCTS = ["BTC-USD","ETH-USD","SOL-USD","ADA-USD","AVAX-USD","DOGE-USD","XRP-USD","LTC-USD","LINK-USD","MATIC-USD"];
...
const pct = (from?: number, to?: number) => (from && to && from !== 0) ? ((to - from) / from) * 100 : undefined;
const bodyRatio = (c: Candle) => { const r = c.high - c.low; return r>0 ? Math.abs(c.close - c.open) / r : 0; };
...
async function fetchCoinbaseTickers(): Promise<Record<string, number>> {
  ...
  await Promise.all(PRODUCTS.map(async (p) => {
    try {
      ...
      if (j?.price) out[p] = parseFloat(j.price);
    } catch {}
  }));
  return out;
}
...
function computeSignals(args: {
  ...
  for (const sym of Object.keys(args.latest)){
    const nowC = args.latest[sym], c1 = args.c1m[sym], c3 = args.c3m[sym];
    if (!nowC || !c1 || !c3) continue;
    const p1 = pct(c1.close, nowC.close);
    const p3 = pct(c3.close, nowC.close);
    if (p1==null || p3==null) continue;
    ...
    if (Math.abs(p1) < 0.8 || Math.abs(p3) < 1.2 || vz < 1.0 || b < 0.35) continue;
    const direction = p1 > 0 ? "PUMP" : "DUMP";
    let x = (0.6*(p1/1.0)) + (0.4*(p3/1.0)) + (0.8*vz) + (0.2*st) + (0.5*((b-0.35)*3));
    if (direction==="DUMP") x = -x;
    const score = sigmoid(x);
    const tags: string[] = [];
    if (Math.abs(p1) >= 2) tags.push("fast-move");
    if (Math.abs(p3) >= 3) tags.push("impulse-3m");
    if (vz >= 2) tags.push("vol-spike");
    if (Math.abs(st) >= 3) tags.push("streak");
    out.push({ symbol:sym, direction, score:+score.toFixed(3), pct_1m:+p1.toFixed(3), pct_3m:+p3.toFixed(3), vol_z:+vz.toFixed(2), streak:st, body:+b.toFixed(2), tags, ts: now });
  }
...
async function buildBundle(env: Env, prices: Record<string, number>): Promise<DataBundle> {
  ...
  for (const [sym, price] of Object.entries(prices)){
    ...
    const base: Candle = prev || { ts: now, open: price, high: price, low: price, close: price, volume: 0 };
    const high = Math.max(base.high, price);
    const low  = Math.min(base.low, price);
    latest[sym] = { ts: now, open: base.open, high, low, close: price, volume: (base.volume||0) + 1 };
  }
  ...
  const by = (key: keyof MarketRow, asc=false)=> [...rows].filter(r=> r[key] != null).sort((a,b)=>{
...
function allowedThreshold(s: PumpDump){ return s.score >= 0.85 && s.vol_z >= 2; }
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (req.method === 'GET' && url.pathname === '/signals/pumpdump') {
      ...
      if (!s) return bad('no signals', 404);
      return new Response(s, { headers: { 'content-type': 'application/json' } });
    }
    ...
    if (req.method === 'POST' && url.pathname === '/mock/seed') {
      ...
      if (key !== env.CRON_SECRET) return bad('unauthorized', 401);
      const now = Date.now();
      ...
      const signals: PumpDump[] = [
        { symbol:'SOL-USD', direction:'PUMP', score:0.91, pct_1m:1.3, pct_3m:2.4, vol_z:2.1, streak:3, body:0.6, tags:['vol-spike','streak'], ts: now }
      ];
  ...
  async scheduled(controller: ScheduledController, env: Env) {
    ...
    const vstats = Object.fromEntries(Object.entries(vstatsFull).map(([k,v]:any)=>[k,{mean:v.mean,std:v.std}]));
    ...
    const trigSyms = new Set(signals.filter(s=> s.score >= 0.85 && s.vol_z >= 2).map(s=>s.symbol));
    if (trigSyms.size){
      ...
      for (const dev of devices){
        ...
        const wlJson = await wlRes.json().catch(()=>({watchlist:[]}));
        ...
        if (watch.some(s=>trigSyms.has(s))){
          ...
          if (devData?.expoToken){
            await env.ALERTS_Q.send({ token: devData.expoToken, title: "Moonwalking Signal", body: "Watchlist pump/dump triggered", ts: Date.now() });
          }
        }
      }
    }
    ...
    for (const c of connectors){
      try { posts = posts.concat(await c.pullSince(lastTs)); } catch {}
    }
