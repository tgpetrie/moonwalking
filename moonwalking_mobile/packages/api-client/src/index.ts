...
export const createClient = (baseURL: string, deviceId: string) => {
  ...
  return {
    ...
    async addWatch(symbol: string) { return (await client.post('/watchlist/add', { symbol })).data; },
    async removeWatch(symbol: string) { return (await client.post('/watchlist/remove', { symbol })).data; },
    async getWatch() { return (await client.get('/watchlist')).data; }
  };
};
