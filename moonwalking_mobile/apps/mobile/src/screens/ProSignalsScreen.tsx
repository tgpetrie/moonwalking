    ...
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 16 }}>
      ...
      {data?.length ? data.map((s)=> (
        <View key={s.symbol + s.ts} style={styles.card}>
          ...
          <Text style={s.direction==='PUMP'?styles.pump:styles.dump}>{s.direction} — score {s.score.toFixed(2)}</Text>
          <Text style={styles.meta}>1m {s.pct_1m}% • 3m {s.pct_3m}% • vz {s.vol_z} • px {s.streak}</Text>
