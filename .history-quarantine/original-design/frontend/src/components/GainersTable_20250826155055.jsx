// File: original-design/frontend/src/lib/watchlist.js
// Centralized watchlist helpers with optional Codex integration.
// Tries Codex first (if present), falls back to localStorage.
...
export function saveWatchlist(list) {
  ...
  try {
    ...
    window.dispatchEvent(new CustomEvent('watchlist:changed', { detail: uniq }));
  } catch {}
  return uniq;
}
...
export function addToWatchlist(sym) {
  const S = _norm(sym);
  const cur = new Set(loadWatchlist());
  if (!cur.has(S)) cur.add(S);
  return saveWatchlist(Array.from(cur));
}
...
export function toggleWatchlist(sym) {
  const S = _norm(sym);
  const cur = new Set(loadWatchlist());
  if (cur.has(S)) cur.delete(S); else cur.add(S);
  saveWatchlist(Array.from(cur));
  return cur.has(S);
}

// File: original-design/frontend/src/components/GainersTable.jsx
import React, { useEffect, useMemo, useState } from 'react';
import { io } from 'socket.io-client';
import { loadWatchlist, toggleWatchlist, loadWatchlistItems, pctSinceAdded } from '../lib/watchlist.js';

// If you have API helpers already, wire them here; fallback to env/default
...
function normalize(raw, key) {
  ...
  for (const r of rows) {
    ...
    if (!sym) continue;
    const pct = Number(
      r[key] ?? r[`change_${key}`] ?? r[`pct_${key}`] ?? r[`delta_${key}`] ?? r.change ?? 0
    ) || 0;
            ...
            <div key={r.symbol} className="tile">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const nowIn = toggleWatchlist(r.symbol, r.price);
...
export default function Watchlist() {
  ...
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === 'watchlist:symbols') {
        setSymbols(loadWatchlist());
        setItems(loadWatchlistItems());
      }
    };
    window.addEventListener('storage', onStorage);
    return () => window.removeEventListener('storage', onStorage);
  ...
  // If empty, render nothing (per original behaviour)
  ...
  return (
    <div className="panel">
      ...
      <ul className="watchlist">
        {items.map(it => (
          <li key={it.symbol} className="watchlist-item">
            <div className="mono" style={{fontWeight:600}}>{it.symbol}</div>
            <div className="meta small">
              {it.addedPrice ? `added ${fmtPrice(it.addedPrice)}` : 'added —'}
              {it.addedPrice && it.addedAt ? (
                <span style={{marginLeft:8}}>{(() => {
                  const pct = pctSinceAdded(it.addedPrice, /* currentPrice unknown here */ it.addedPrice);
