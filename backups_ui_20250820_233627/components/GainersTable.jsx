...
function normalizeGainers(payload) {
  if (!payload) return [];
  if (Array.isArray(payload)) return payload;
  if (payload?.data && Array.isArray(payload.data)) return payload.data;
  if (payload?.crypto && Array.isArray(payload.crypto)) return payload.crypto;
  if (payload?.crypto_meta?.gainers && Array.isArray(payload.crypto_meta.gainers)) return payload.crypto_meta.gainers;
  if (payload?.component && payload?.items && Array.isArray(payload.items)) return payload.items;
  if (payload?.gainers && Array.isArray(payload.gainers)) return payload.gainers;
  return [];
}

function dedupeByBestChange(rows) {
  const bySym = new Map();
  for (const r of rows) {
...
export default function GainersTable() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const payload = await fetchWithSWR(API_ENDPOINTS.t3m);
        if (!alive) return;
        const norm = dedupeByBestChange(normalizeGainers(payload));
        // keep only positive changes
        const onlyPos = norm.filter(r => Number(r._pct) > 0);
        setRows(onlyPos);
      } catch (e) {
        if (!alive) return;
  ...
  if (err) {
    return <div className="p-2 text-sm text-red-500">3‑min gainers error: {err}</div>;
  }
  if (!rows?.length) {
    return <div className="p-2 text-sm text-slate-400">No 3‑min gainers data available.</div>;
  }
      ...
      <table className="w-full text-sm app-table">
        <thead>
          <tr className="text-left text-slate-400">
            <th className="py-2 pr-4 w-14 pl-4">#</th>
          ...
          {rows.map((r, i) => {
            ...
            return (
              <tr key={`${sym}-${i}`} className={i%2 ? 'bg-zinc-900/20' : ''}>
                <td className="py-2 pl-4 text-zinc-400">{i+1}</td>
