...
const LosersTable = ({ refreshTrigger }) => {
  ...
  // Fetcher for losers with diff annotation
  const fetcher = async () => {
    ...
    .sort((a,b) => a.change - b.change) // losers most negative first
    .slice(0,7);
  ...
  const { data: polled, error: pollError, loading: pollLoading } = useStaggeredPolling(fetcher, {
    interval: 30000,
    offset: 5000, // start 5s after gainers to stagger load
    jitter: 1500,
    active: true
  });

  const visibleRows = useStaggeredRows(polled || [], 55, 120); // slight initial delay
              ...
              <div className="relative overflow-hidden rounded-xl p-4 hover:scale-[1.02] sm:hover:scale-[1.035] transition-transform">
                ...
                <TableShell>
                  ...
                  <div className="w-[108px] pr-1.5 text-right align-top">
                    <div className={`text-base sm:text-lg md:text-xl font-bold font-mono leading-none whitespace-nowrap ${r.change > 0 ? 'text-[#C026D3]' : 'text-pink'} ${changeFlashClass}`}> 
