...
# Usage: start-5173-5001.sh [--no-backend]
...
# Atomic write of frontend port so backend reads a consistent file
...
# Start backend (use your local start if present) unless flagged not to
if [[ $NO_BACKEND -eq 0 && -d "$BACKEND_DIR" ]]; then
  ...
  echo "Skipping backend start (NO_BACKEND=$NO_BACKEND)"
fi
