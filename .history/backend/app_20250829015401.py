...
def create_session():
    ...
    # Increase pool size and enable blocking on pool exhaustion so
    # callers wait briefly for a connection instead of silently
    # discarding one (which causes the warning seen in logs).
    # Tune these values depending on expected concurrency.
    adapter = HTTPAdapter(
...
# Bound concurrency for outbound Coinbase requests. User requested "usually 50".
# This prevents aggressive bursts from exhausting the urllib3 connection pool.
_COINBASE_CONCURRENCY = int(os.environ.get('COINBASE_CONCURRENCY', '50'))
_coinbase_semaphore = threading.BoundedSemaphore(_COINBASE_CONCURRENCY)

def safe_coinbase_get(url, timeout=None, params=None, **kwargs):
    """Thread-safe wrapper around api_session.get that bounds concurrency.

    Acquires a bounded semaphore before issuing the request so at most
    _COINBASE_CONCURRENCY requests are active concurrently. If acquiring
    the semaphore is interrupted or times out, the function will try once
    more after a short backoff and otherwise raise the original exception.
    """
    acquired = False
    try:
        # Wait up to a short period for a slot; this avoids blocking forever
        # if something went wrong. Use a small timeout so callers fail fast.
        acquired = _coinbase_semaphore.acquire(timeout=5)
        if not acquired:
            # Failed to acquire in time; try once more after tiny sleep
            time.sleep(0.1)
            acquired = _coinbase_semaphore.acquire(timeout=2)
            if not acquired:
                raise RuntimeError("Timed out waiting for Coinbase request slot")

        # Perform the request using the shared session
        return api_session.get(url, params=params, timeout=timeout, **kwargs)
    finally:
        if acquired:
            try:
                _coinbase_semaphore.release()
            except Exception:
                pass
...
CONFIG = {
    # Core cadences
    'CACHE_TTL': int(os.environ.get('CACHE_TTL', 15)),  # Reduce to 15s so data actually refreshes each UI tick
    ...
    'UPDATE_INTERVAL': int(os.environ.get('UPDATE_INTERVAL', 15)),  # Background update interval now 15s
    ...
    'ONE_MIN_REFRESH_SECONDS': int(os.environ.get('ONE_MIN_REFRESH_SECONDS', 15)),  # Recompute 1-min snapshot every 15s to align with UI
...
# Lightweight 1-hour dataset cache (built from 24h movers API which is heavier). TTL > 15s to limit upstream load.
one_hour_cache = {"data": None, "timestamp": 0, "ttl": int(os.environ.get('ONE_HOUR_TTL', 60))}
...
def _tcp_probe(host: str, port: int, timeout_ms: int = 800) -> bool:
    """Return True if TCP connect to host:port succeeds within timeout_ms milliseconds."""
    try:
...
def get_coinbase_prices():
    ...
    try:
        products_response = safe_coinbase_get(COINBASE_PRODUCTS_URL, timeout=CONFIG['API_TIMEOUT'])
        ...
        major_coins = [
            'BTC-USD', 'ETH-USD', 'SOL-USD', 'ADA-USD', 'DOT-USD', 'LINK-USD', 'MATIC-USD',
            'AVAX-USD', 'ATOM-USD', 'ALGO-USD', 'XRP-USD', 'DOGE-USD', 'SHIB-USD', 'UNI-USD',
            'AAVE-USD', 'BCH-USD', 'LTC-USD', 'ICP-USD', 'HYPE-USD', 'SPX-USD', 'SEI-USD',
            'PI-USD', 'KAIA-USD', 'INJ-USD', 'ONDO-USD', 'CRO-USD', 'FLR-USD', 'WLD-USD',
            'POL-USD', 'WBT-USD', 'JUP-USD', 'SKY-USD', 'TAO-USD'
        ]
        ...
        for product in usd_products:
            if product.get('id') in major_coins:
                prioritized_products.append(product)
            else:
                remaining_products.append(product)

        # Combine prioritized + remaining, but limit total to 100 for speed
        limit = max(0, 100 - len(prioritized_products))
        all_products = prioritized_products + remaining_products[:limit]
        ...
        def fetch_ticker(product):
            ...
            try:
                ticker_response = safe_coinbase_get(ticker_url, timeout=1.5)
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_product = {executor.submit(fetch_ticker, product): product for product in all_products[:50]}
            for future in as_completed(future_to_product):
                try:
                    ...
                    logging.warning(f"Ticker future error: {fe}")
    ...
    try:
        ...
        def fetch_product_data(product):
            ...
            try:
                ...
                stats_response = safe_coinbase_get(stats_url, timeout=3)
                if stats_response.status_code != 200:
                    return None

                # Get current price
                ticker_url = f"https://api.exchange.coinbase.com/products/{product['id']}/ticker"
                ticker_response = safe_coinbase_get(ticker_url, timeout=2)
...
def build_one_hour_dataset():
    """Return a lightweight 1-hour change dataset (cached) with canonical keys.
    Source: 24h movers list which already includes an estimated 1h change.
    """
    now = time.time()
    if one_hour_cache["data"] and (now - one_hour_cache["timestamp"]) < one_hour_cache.get("ttl", 60):
        return one_hour_cache["data"]
    ...
    for c in movers:
        try:
            rows.append({
                "symbol": c["symbol"],
                # canonical 1-hour view keys (using current + initial_1h estimate)
                "current": c["current_price"],
                "initial_1h": c.get("initial_price_1h"),
                "gain": c.get("price_change_1h", 0),
                "interval_minutes": 60,
                # legacy style fields (if UI reuses generic row renderer)
                "current_price": c["current_price"],
                "price_change_percentage_1h": c.get("price_change_1h", 0),
            })
        except Exception:
            continue
    # Sort by absolute 1h move, cap set size for consistent layout
    rows.sort(key=lambda r: abs(r.get("gain", 0)), reverse=True)
    rows = rows[:30]
    payload = {"rows": rows, "count": len(rows), "ttl": one_hour_cache.get("ttl", 60)}
    one_hour_cache["data"] = payload
    one_hour_cache["timestamp"] = now
    return payload
...
def get_historical_chart_data(symbol, days=7):
    ...
    try:
        ...
        response = safe_coinbase_get(url, params=params, timeout=CONFIG['API_TIMEOUT'])
...
def get_top_banner_scroll():
    ...
    try:
        ...
        return jsonify({
            ...
            "focus": "price_change",
            "scroll_speed": "medium",
            "update_interval": 15000,  # 15s UI polling (data may internally refresh less often)
            "last_updated": datetime.now().isoformat()
        })
    except Exception as e:
    ...
    try:
        ...
        for i, coin in enumerate(gainers[:20]):  # Top 20 gainers
            ...
            gainers_table_data.append({
                "rank": i + 1,
                "symbol": coin["symbol"],
                # canonical keys for new UI contract
    ...
    try:
        ...
        for i, coin in enumerate(gainers[:20]):  # Top 20 gainers
            gainers_table_data.append({
                ...
                # canonical keys
                "current": coin["current"],
                "initial_1min": coin["initial_1min"],
                "gain": coin["gain"],
                "interval_minutes": coin.get("interval_minutes", 1),
                "current_price": coin["current"],
...
@app.route('/api/component/snapshot')
def unified_snapshot():
    """Unified snapshot delivering 1-min, 3-min and 1-hour datasets in a single call.
    Contract:
      {
        "snapshot_at": iso timestamp,
        "update_interval": 15000,
        "one_min": { gainers: [...], losers: [...] },
        "three_min": { gainers: [...], losers: [...] },
        "one_hour": { rows: [...] }
      }
    All row objects use canonical keys: symbol, current, initial_[interval], gain, interval_minutes.
    """
    try:
        three = get_crypto_data() or {}
        one_m = get_crypto_data_1min() if CONFIG.get('ENABLE_1MIN', True) else None
        one_h = build_one_hour_dataset()
        return jsonify({
            "snapshot_at": datetime.now().isoformat(),
            "update_interval": 15000,
            "one_min": {
                "gainers": one_m.get('gainers', []) if one_m else [],
                "losers": one_m.get('losers', []) if one_m else [],
                "throttled": bool(one_m and one_m.get('throttled'))
            },
            "three_min": {
                "gainers": three.get('gainers', [])[:15],
                "losers": three.get('losers', [])[:15]
            },
            "one_hour": one_h,
            "schema": {
                "canonical_keys": ["symbol", "current", "initial_1min", "initial_3min", "initial_1h", "gain", "interval_minutes"],
                "update_interval_seconds": 15
            }
        })
    except Exception as e:
        logging.error(f"Error building unified snapshot: {e}")
...
def health_check():
    ...
    try:
        ...
        try:
            coinbase_response = safe_coinbase_get("https://api.exchange.coinbase.com/products", timeout=5)
...
def server_info():
    ...
    try:
        ...
        # If a frontend port file exists, surface it for local dev tooling
        try:
            frontend_port_path = os.path.join(os.path.dirname(__file__), 'frontend.port')
            if os.path.exists(frontend_port_path):
                with open(frontend_port_path, 'r') as f:
                    port_txt = f.read().strip()
                    if port_txt:
                        try:
                            fp = int(port_txt)
                            payload['frontend_port'] = int(port_txt)
                            payload['frontend_url'] = f"http://localhost:{payload['frontend_port']}"
                            # Probe the frontend briefly to ensure it's actually listening
                            try:
                                listening = _tcp_probe('127.0.0.1', fp, timeout_ms=800)
                                payload['frontend_listening'] = bool(listening)
                            except Exception:
                                payload['frontend_listening'] = False
                        except ValueError:
                            # ignore malformed port file
                            pass
        except Exception:
            # non-fatal: don't block server-info on this
            pass
