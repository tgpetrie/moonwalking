...
TMP_PORT_FILE="$BACKEND_DIR/frontend.port.$$"
printf '%s' "$FRONTEND_PORT" > "$TMP_PORT_FILE"
mv -f "$TMP_PORT_FILE" "$BACKEND_DIR/frontend.port"
...
(
  # Prefer the full-featured original-design backend if available
  ORIGINAL_BACKEND_DIR="$ROOT_DIR/original-design/backend"
  if [[ -d "$ORIGINAL_BACKEND_DIR" && -f "$ORIGINAL_BACKEND_DIR/app.py" ]]; then
    START_DIR="$ORIGINAL_BACKEND_DIR"
    echo "[restart_dev] using original-design backend at $START_DIR"
  else
...
chmod +x "$FE_WRAPPER"
# call FE wrapper but tell it not to start backend again
FRONTEND_PORT="$FRONTEND_PORT" BACKEND_PORT="$BACKEND_PORT" "$FE_WRAPPER" --no-backend
