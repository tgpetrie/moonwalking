...
function fmtPrice(n){ if(!Number.isFinite(n)) return '-'; const v=Number(n); return `$${v.toFixed(v>=1?2:4)}`; }
function fmtPct(n){ if(!Number.isFinite(n)) return '-'; const s = n>0?'+':''; return `${s}${n.toFixed(2)}%`; }

function normalize(raw){
  const rows = Array.isArray(raw)? raw : raw?.data || raw?.rows || [];
  return (rows||[]).map(r=>({ symbol:(r.symbol||r.ticker||r.s||r.base||'').toUpperCase(), price: Number(r.price||r.last||r.p||r.close||0), pct: Number(r.change||r.pct||0) }))
    .filter(x=>x.symbol).sort((a,b)=>b.pct-a.pct);
}
...
export default function GainersTable({ windowMinutes=3, view='table' }){
  ...
  useEffect(()=>{
    let cancelled=false;
    (async()=>{
      try{ const res = await fetch(endpoint); if(res.ok){ const j=await res.json(); if(!cancelled) setRows(normalize(j)); }}catch{}
    })();
    return ()=>{ cancelled=true };
  },[endpoint]);

  const onAdd = async (symbol, price)=>{
    try{ const { toggleWatchlist, loadWatchlist: _load } = await import('../lib/watchlist'); await toggleWatchlist(symbol, price); setWatchlist(new Set(_load())); }catch{}
  };

  if(!rows.length) return <div className="panel"><div className="panel__header"><h3>Gainers</h3></div><div className="empty">No data</div></div>;

  return (
    <div className="panel">
      <div className="panel__header"><h3>Gainers</h3><div className="meta">{rows.length} assets</div></div>
      <table className="table compact"><thead><tr><th>Symbol</th><th>Price</th><th>Δ</th><th/></tr></thead>
        <tbody>{rows.map(r=> (
          <tr key={r.symbol}><td className="mono">{r.symbol}</td><td className="num">{fmtPrice(r.price)}</td><td className={`num ${r.pct>=0?'positive':'negative'}`}>{fmtPct(r.pct)}</td>
            <td style={{textAlign:'right'}}><StarIcon filled={watchlist.has(r.symbol)} onClick={()=>onAdd(r.symbol, r.price)} /></td></tr>
        ))}</tbody>
      </table>
    </div>
  );
