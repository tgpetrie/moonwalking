            ...
            <div className="flex flex-row items-center gap-3 sm:gap-4 w-full sm:w-auto">
              ...
              <div className={`flex flex-col items-end min-w-[56px] sm:min-w-[60px]`}>
                  <div className={`flex items-center gap-1 font-bold text-base sm:text-lg md:text-xl ${change >= 0 ? 'text-purple' : 'text-red-400'}`}>
                    <span className="numeric">{Number.isFinite(change) ? `${change > 0 ? '+' : ''}${formatPercentage(change)}` : 'N/A'}</span>
                  </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        <div className="space-y-1">
          {leftCol.map((item, i) => {
            ...
            const isWatched = Array.isArray(topWatchlist) && topWatchlist.includes(symbol);
            return <UniformCard key={`${symbol}-${rank}`} symbol={symbol} price={price} change={change} rank={rank} peak={peak} showPeak={true} windowLabel="1-min" filled={isWatched} onToggle={handleToggleWatchlist} />;
          })}
        </div>
        <div className="space-y-1">
          {rightCol.map((item, i) => {
      ...
      {showMore && (
        <div className="mt-3 space-y-1">
          {slicedData.slice(8).map((item, idx) => {
            ...
            const isWatched = Array.isArray(topWatchlist) && topWatchlist.includes(symbol);
            return <UniformCard key={`${symbol}-${rank}`} symbol={symbol} price={price} change={change} rank={rank} showPeak={false} windowLabel="1-min" filled={isWatched} onToggle={handleToggleWatchlist} />;
          })}
        </div>
      )}
