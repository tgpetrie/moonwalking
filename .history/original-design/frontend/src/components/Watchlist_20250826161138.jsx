      ...
      await Promise.all(
        syms.map(async (s) => {
          try {
            ...
            const amt = json && json.data && json.data.amount ? Number(json.data.amount) : null;
            if (!cancelled) {
              pmap[s] = amt;
            }
          } catch (e) {
  ...
  return (
    <div className="panel">
      <div className="panel__header">
        <h3>My Watchlist</h3>
        <div className="meta">assets {items.length}</div>
      </div>
...
export default function Watchlist() {
  ...
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === 'watchlist:symbols') setItems(loadWatchlistItems());
    };
    window.addEventListener('storage', onStorage);
