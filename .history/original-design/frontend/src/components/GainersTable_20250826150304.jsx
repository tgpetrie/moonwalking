        ...
        return (
          <React.Fragment key={item.symbol}>
            <div className="relative group">
              ...
              >
                <div
                  className={`table-card flex items-center justify-between group-hover:scale-[1.035]`}
                ...
                >
                  ...
                  <div className="flex items-center gap-4">
                    {/* Rank Badge (purple) */}
                    <div className="table-rank table-rank-purple">{item.rank}</div>
                    {/* Symbol */}
                    <div className="flex-1 flex items-center gap-3 ml-4">
                      <span className="font-bold text-white text-lg tracking-wide hover:text-cyan-400 hover:text-shadow-cyan-400 table-title">
                        {item.symbol}
                      </span>
                      {showAdded && (
                  ...
                  <div className="flex flex-row flex-wrap items-center gap-2 sm:gap-4 ml-0 sm:ml-4 w-full sm:w-auto">
                    ...
                    <div className="flex flex-col items-end min-w-[72px] sm:min-w-[100px] ml-2 sm:ml-4">
                      <span className="text-base sm:text-lg md:text-xl font-bold text-teal select-text numeric">
                      ...
                      <span className="text-xs sm:text-sm md:text-base font-light text-gray-400 select-text">
                        {Number.isFinite(item.price) && Number.isFinite(item.change) && item.change !== 0
                          ? (() => {
                    ...
                    <div className="flex flex-col items-end min-w-[56px] sm:min-w-[60px]">
                      <div className={`flex items-center gap-1 font-bold text-base sm:text-lg md:text-xl ${item.change > 0 ? 'text-purple' : 'text-pink'}`}> 
                        <span className="numeric">{typeof item.change === 'number' ? formatPercentage(item.change) : 'N/A'}</span>
                      </div>
                      <span className="text-xs sm:text-sm md:text-base font-light text-gray-400">
                        3-Min
                      </span>
                    </div>
                    {/* dot indicators removed per design */}
                    <button
