      ...
      <div className="panel">
        ...
        <div className="gainers-tiles">
          {top8.map((r, idx) => {
            ...
            const change = Number(r.pct || r.change || 0);
            const rank = idx + 1;
            const isWatched = watchlist.has(symbol);
            return <UniformCard key={symbol} symbol={symbol} price={price} change={change} rank={rank} showPeak={false} windowLabel="3-min" filled={isWatched} onToggle={onAddToWatchlist} />;
          })}
        </div>
      </div>
    );
