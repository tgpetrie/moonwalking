...
function getCurrentPrices(symbols) {
  ...
  symbols.forEach((symbol) => {
    prices[symbol] = Number(parseFloat((Math.random() * 1000).toFixed(2))); // Simulated price (numeric)
  });
  return prices;
}
...
const Watchlist = ({ onWatchlistChange, topWatchlist, quickview }) => {
  ...
  useEffect(() => {
    if (typeof topWatchlist !== 'undefined') {
      // Normalize incoming topWatchlist items so numeric fields exist
      setWatchlist(topWatchlist.map(item => ({
        ...item,
        priceAtAdd: typeof item.priceAtAdd === 'number' ? item.priceAtAdd : (typeof item.price === 'number' ? item.price : 0),
        currentPrice: typeof item.currentPrice === 'number' ? item.currentPrice : (typeof item.price === 'number' ? item.price : (typeof item.lastPrice === 'number' ? item.lastPrice : 0))
      })));
      setLoading(false);
    } else {
      setLoading(true);
      try {
        ...
        // Ensure saved items have numeric priceAtAdd and currentPrice
        setWatchlist(data.map((item) => ({
          ...item,
          priceAtAdd: typeof item.priceAtAdd === 'number' ? item.priceAtAdd : (typeof item.price === 'number' ? item.price : 0),
          currentPrice: typeof item.currentPrice === 'number' ? item.currentPrice : (typeof item.priceAtAdd === 'number' ? item.priceAtAdd : (typeof item.price === 'number' ? item.price : 0)),
        })));
  ...
  const handleAdd = () => {
    ...
    if (!symbol) return;
  const fakePrice = Number(parseFloat((Math.random() * 1000).toFixed(2))); // Replace with real price fetch
      ...
      {filteredWatchlist.length > 0 ? (
        <div className="flex flex-col gap-2 items-start">
          {filteredWatchlist.map((item) => {
            // Coerce numeric fields to numbers with safe fallbacks
            const priceAtAdd = Number(item.priceAtAdd ?? 0) || 0;
            const priceNow = Number(item.currentPrice ?? priceAtAdd) || priceAtAdd;
            ...
            return (
              <div key={item.symbol} className="flex items-center justify-between w-full p-3 sm:p-4 rounded-xl transition-all duration-300 cursor-pointer relative overflow-hidden bg-transparent border border-orange-200/30 min-w-0">
                  <div className="flex flex-col items-start flex-1">
                    ...
                    {(() => {
                      const addedStr = Number.isFinite(priceAtAdd) ? formatPrice(priceAtAdd) : 'N/A';
                      const nowStr = Number.isFinite(priceNow) ? formatPrice(priceNow) : 'N/A';
                      const changeStr = Number.isFinite(change) ? `${change > 0 ? '+' : ''}${formatPercentage(change)}` : 'N/A';
                      return <span className="text-xs text-gray-400 block">Added: {addedStr} | Now: {nowStr} | {changeStr}</span>;
                    })()}
                  </div>
