...
export default function GainersTable({ windowMinutes = 3, view = 'table' }) {
  ...
  useEffect(() => {
    const onStorage = (e) => { if (e.key === 'watchlist:symbols') import('../lib/watchlist').then(({ loadWatchlist }) => setWatchlist(new Set(loadWatchlist()))).catch(() => {}); };
    window.addEventListener('storage', onStorage);
    import('../lib/watchlist').then(({ loadWatchlist }) => setWatchlist(new Set(loadWatchlist()))).catch(() => {});
  ...
  const onToggle = async (symbol, price) => {
    try {
      const { toggleWatchlist, loadWatchlist } = await import('../lib/watchlist');
