      ...
      <div className="relative h-16 overflow-hidden" tabIndex={0} aria-label="Scrolling market data" style={{outline:'none'}}>
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="flex whitespace-nowrap animate-scroll" role="list">
            {/* First set of data */}
            {data.map((coin) => (
              <div key={`first-${coin.symbol}`} className="flex-shrink-0 mx-8 group" role="listitem" tabIndex={0} aria-label={`#${coin.rank} ${coin.symbol}, ${formatPrice(coin.price)}, Vol: ${coin.volume_change >= 0 ? '+' : ''}${formatPercentage(coin.volume_change)}, ${coin.badge}`}>
                <div className="flex items-center gap-4 pill-hover px-4 py-2 rounded-full transition-all duration-300 group-hover:text-purple group-hover:text-shadow-purple focus:ring-2 focus:ring-purple">
                  ...
                  <div className="font-mono text-sm text-teal">{formatPrice(coin.price)}</div>
                  <div className="flex items-center gap-1 text-sm font-bold">
                    <span className={coin.volume_change >= 0 ? 'text-blue' : 'text-pink'}>Vol: {coin.volume_change >= 0 ? '+' : ''}{formatPercentage(coin.volume_change)}</span>
                  </div>
            ...
            {data.map((coin) => (
              <div key={`second-${coin.symbol}`} className="flex-shrink-0 mx-8 group" role="listitem" tabIndex={0} aria-label={`#${coin.rank} ${coin.symbol}, ${formatPrice(coin.price)}, Vol: ${coin.volume_change >= 0 ? '+' : ''}${formatPercentage(coin.volume_change)}, ${coin.badge}`}>
                <div className="flex items-center gap-4 pill-hover px-4 py-2 rounded-full transition-all duration-300 group-hover:text-purple group-hover:text-shadow-purple focus:ring-2 focus:ring-purple">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-purple">#{coin.rank}</span>
                    <span className="text-sm font-headline font-bold tracking-wide">
