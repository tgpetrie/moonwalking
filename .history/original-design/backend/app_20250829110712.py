...
def server_info():
    ...
    # Attempt to discover the frontend port file written by the dev scripts.
    frontend_url = None
    try:
        from pathlib import Path
        candidates = []
        # 1) explicit env override
        fenv = os.getenv('FRONTEND_PORT_FILE')
        if fenv:
            candidates.append(Path(fenv))
        # 2) repo-root backend/frontend.port (three levels up from this file)
        candidates.append(Path(__file__).parent.parent.parent / 'backend' / 'frontend.port')
        # 3) original-design/backend/../backend/frontend.port
        candidates.append(Path(__file__).parent.parent / 'backend' / 'frontend.port')
        # 4) same folder as this script
        candidates.append(Path(__file__).parent / 'frontend.port')
        for p in candidates:
            if p and p.exists():
                try:
                    port_text = p.read_text().strip()
                    if port_text:
                        frontend_url = f"http://localhost:{int(port_text)}"
                        break
                except Exception:
                    continue
    except Exception:
    ...
    if frontend_url:
        resp['frontend'] = { 'url': frontend_url }
    return jsonify(resp)
